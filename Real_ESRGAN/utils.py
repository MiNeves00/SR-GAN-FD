...
def validate(
    ...
    niqes = AverageMeter("NIQE", ":4.2f")
    progress = ProgressMeter(len(data_prefetcher), [batch_time, niqes], prefix=f"{mode}: ")
    ...
    with torch.no_grad():
        while batch_data is not None:
            ...
            niqe = niqe_model(sr, gt)
            niqe.update(niqe.item(), lr.size(0))
    ...
    if mode == "Valid" or mode == "Test":
        writer.add_scalar(f"{mode}/NIQE", niqe.avg, epoch + 1)
    else:
